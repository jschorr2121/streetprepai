...
-- Set search_path for the entire migration session so `vector` resolves
-- correctly in both table column types and function signatures.
-- The extension lives in the `extensions` schema on hosted Supabase.
...
create index if not exists chat_embeddings_cosine_idx
  on public.chat_embeddings using ivfflat (embedding extensions.vector_cosine_ops)
  with (lists = 100);
