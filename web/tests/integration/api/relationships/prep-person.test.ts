/**
 * Integration tests for POST /api/relationships/prep-person — streaming prep sheet.
 */
    ...
    messages: {
      stream: vi.fn().mockReturnValue({
        [Symbol.asyncIterator]: async function* () {
          ...
          yield { type: "content_block_delta", delta: { type: "text_delta", text: "prep" } };
        },
        finalMessage: async () => ({
...
const validBody = {
  ...
  firm: "Goldman Sachs",
  title: "VP",
  bio: "VP at GS TMT, ex-MS analyst.",
};

describe("POST /api/relationships/prep-person", () => {
  it("returns 401 when unauthenticated", async () => {
    ...
    const res = await POST(
      new Request("http://localhost/api/relationships/prep-person", {
        ...
        headers: { "x-forwarded-for": "19.0.0.1", "Content-Type": "application/json" },
      }),
    );
  ...
  it("returns 400 on malformed body", async () => {
    getUserMock.mockResolvedValue(fakeUser({ id: "u-pp-bad" }));
    const { POST } = await import("@/app/api/relationships/prep-person/route");
    const res = await POST(
      new Request("http://localhost/api/relationships/prep-person", {
        method: "POST",
        body: JSON.stringify({ name: "" }),
        headers: { "x-forwarded-for": "19.0.0.2", "Content-Type": "application/json" },
      }),
    );
  ...
  it("streams text on happy path", async () => {
    getUserMock.mockResolvedValue(fakeUser({ id: "u-pp-ok" }));
    const { POST } = await import("@/app/api/relationships/prep-person/route");
    const res = await POST(
      new Request("http://localhost/api/relationships/prep-person", {
        method: "POST",
        body: JSON.stringify(validBody),
        headers: { "x-forwarded-for": "19.0.0.3", "Content-Type": "application/json" },
      }),
    );
  ...
  it("returns 429 after exhausting per-user budget", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      const res = await POST(
        new Request("http://localhost/api/relationships/prep-person", {
          method: "POST",
          body: JSON.stringify(validBody),
          headers: { "x-forwarded-for": `19.1.${i}.1`, "Content-Type": "application/json" },
        }),
      );
      if (res.body) {
        const reader = res.body.getReader();
    ...
    const denied = await POST(
      new Request("http://localhost/api/relationships/prep-person", {
        method: "POST",
        body: JSON.stringify(validBody),
        headers: { "x-forwarded-for": "19.1.99.99", "Content-Type": "application/json" },
      }),
    );
